...
public class UtGen {
	...
	private static int instruccionMasAlta = 0; // Almacena la direccion de la instruccion que ha resultado ser la mayor
												// hasta ahora
	 ...
	 * al tope de la memoria (para almacenamiento de variable temporales como t1,
	 * t2, etc.)
	 */
	...
	/*
	 * La funcion restaurar respaldo, restaura la direccion de emision de codigo
	 * actual
	...
	public static void emitirRM_Abs(String op, int r, int a, String c) {
		Imprimir((instruccionActual) + ":       " + op + "       " + r + "," + (a - (instruccionActual + 1))
				+ "(" + PC + ")");
		++instruccionActual;
		if (debug)
			Imprimir("      " + c);
